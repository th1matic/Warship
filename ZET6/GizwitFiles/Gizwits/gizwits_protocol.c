...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuedoctor_beat != cur->valuedoctor_beat)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedoctor_beat Changed\n");
            ret = 1;
        }
    }
    if(last->valuedoctor_weight != cur->valuedoctor_weight)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedoctor_weight Changed\n");
            ret = 1;
        }
    }
    if(last->valuedoctor_height != cur->valuedoctor_height)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedoctor_height Changed\n");
            ret = 1;
        }
    }
    if(last->valuedoctor_stress != cur->valuedoctor_stress)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedoctor_stress Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuedoctor_beat = gizY2X(doctor_beat_RATIO,  doctor_beat_ADDITION, dataPoints->valuedoctor_beat); 

    devStatusPtr->valuedoctor_weight = exchangeBytes(gizY2XFloat(doctor_weight_RATIO,  doctor_weight_ADDITION, dataPoints->valuedoctor_weight));     
    devStatusPtr->valuedoctor_height = exchangeBytes(gizY2XFloat(doctor_height_RATIO,  doctor_height_ADDITION, dataPoints->valuedoctor_height));     
    devStatusPtr->valuedoctor_stress = exchangeBytes(gizY2XFloat(doctor_stress_RATIO,  doctor_stress_ADDITION, dataPoints->valuedoctor_stress));     
